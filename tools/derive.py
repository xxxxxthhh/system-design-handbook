...
<p class="lead">下面这把尺子不是给面试官用的，是给你自己练完一题后回看用的。每一档写的是<strong>可观察的行为</strong>，不是形容词——你应该能对照自己刚才说的话，逐条打勾。</p>
<div class="tbl-wrap">
<table class="tbl">
...
<tr><td>架构与数据流</td><td>画出了组件框图，但一个请求怎么从头走到尾说不清楚。</td><td>边画边讲清楚了完整数据流，并标出了读路径和写路径的差异。</td><td>主动标出了架构里当前最可能先倒下的那条边，并说明了为什么先怀疑它而不是别的组件。</td></tr>
...
<tr><td>权衡表达</td><td>选了一个方案，被追问「为什么不选另一个」时说不出代价。</td><td>对每个候选方案都说出了具体代价（不是「更贵」这种模糊词，而是一句可核实的账单）。</td><td>说出了「什么事实一变，这个选择就会反过来」——翻转变量式的表达，而不是记住一个固定偏好。</td></tr>
</tbody>
</table>
</div>
